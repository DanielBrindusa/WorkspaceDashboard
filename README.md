...
- On Tasks, the quick views (My Focus, Today, This Week, Overdue, Waiting, Completed) combine with task search, status, area, priority, folder, project, and the same due/active time views and three sort levels. Task Active windows use Start date when set, otherwise Created date, through Due date. An Active time view requires both ends of that range. Weeks run Monday through Sunday for both tasks and projects; a task without a linked project can be selected using No project or No folder.
- Dashboard Projects and Tasks share a limit of five named saved views. A view stores the current filters and three sorting levels in the current account's Firestore workspace. Select one to restore it. When all five slots are used, choose an existing saved view to replace before saving a new one. Removing a saved view does not change projects or tasks.
...
## Waiting, daily review, completion history and Google Calendar

- **Waiting for** lists open follow ups and tasks in Waiting status. A follow up has a project, optional linked contact, person/team, waiting-since date, and follow-up date. Add one from **+ Add → Follow up**, the Waiting for screen, or a project. For a waiting task, edit the task to enter the person/team and follow-up date. Resolve a follow up from its details.
- **Dashboard → Daily review** shows overdue tasks, tasks due this week, pending waits, inbox items, and current projects with no next action. Counts navigate to the appropriate screen.
- Completing an existing task from its status dropdown opens a small form for an optional result and next step. If filled, the completion and its Activity entry are written together; the task's details retain both values. The task edit form also accepts result and next step when changing status to Done.
- Calendar integration is **opt in for each item**. Open a task or project with a due date, or a follow up with a follow-up date, then select **Connect calendar**. Approve the additional Google Calendar scopes using the same account used to sign in. The app discovers a writable calendar named **WorkspaceDashboard** in that account and reads its existing event labels. Choose a label and **Add to calendar**. Event start/end are all day; default reminders are disabled, custom reminder overrides are empty, and attendee update emails are disabled. An event can be updated or removed from the item's calendar panel. Calendar dates do not silently update when a workspace date changes: use **Update calendar event** after editing the item. Removing a workspace item does not automatically delete its Google Calendar event.
- The Google Calendar integration needs one-time setup in the same Google Cloud project as Firebase: enable **Google Calendar API** under **APIs & Services → Library**, then add Calendar scopes `https://www.googleapis.com/auth/calendar.events` and `https://www.googleapis.com/auth/calendar.readonly` under the project's **Google Auth Platform → Data Access** (or OAuth consent screen, depending on the Console layout). If the OAuth app is in Testing, add your Gmail address to **Test users**. Google may request verification before unlisted public users can authorize these scopes. No client secret or service account is needed in the website. Firebase login itself requests only the standard sign-in scope; Calendar permissions are requested only on **Connect calendar**.
